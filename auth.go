...
func (a *Auth) CreateToken(username string, remember bool) string {
	...
	s, _ := token.SignedString([]byte(a.data.JWTSecret))
	return s
}
...
func (a *Auth) VerifyToken(tokenStr string) (string, error) {
	token, err := jwt.Parse(tokenStr, func(t *jwt.Token) (any, error) {
		return []byte(a.data.JWTSecret), nil
	})
...
func (a *Auth) GetEmail() string { return a.data.Admin.Email }
...
// SMTP methods
func (a *Auth) GetSMTP() SMTPConfig { return a.data.SMTP }
