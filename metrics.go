...
func handleMetrics(w http.ResponseWriter, r *http.Request) {
	...
	b.WriteString("# HELP modelmux_requests_total Total number of requests processed.\n")
	...
	b.WriteString("# HELP modelmux_request_errors_total Total number of failed requests.\n")
	b.WriteString("# TYPE modelmux_request_errors_total counter\n")
	b.WriteString(fmt.Sprintf("modelmux_request_errors_total %d\n", metrics.requestErrors.Load()))

	b.WriteString("# HELP modelmux_tokens_total Total tokens consumed.\n")
	b.WriteString("# TYPE modelmux_tokens_total counter\n")
	b.WriteString(fmt.Sprintf("modelmux_tokens_total %d\n", metrics.tokenUsage.Load()))

	b.WriteString("# HELP modelmux_uptime_seconds Uptime in seconds.\n")
	b.WriteString("# TYPE modelmux_uptime_seconds gauge\n")
	b.WriteString(fmt.Sprintf("modelmux_uptime_seconds %.0f\n", time.Since(metrics.startTime).Seconds()))

	// Per-model requests
	b.WriteString("# HELP modelmux_requests_by_model Requests by model.\n")
	b.WriteString("# TYPE modelmux_requests_by_model counter\n")
	metrics.mu.RLock()
	...
	for _, model := range models {
		b.WriteString(fmt.Sprintf("modelmux_requests_by_model{model=%q} %d\n", model, metrics.requestByModel[model].Load()))
	}

	// Per-provider requests
	b.WriteString("# HELP modelmux_requests_by_provider Requests by provider.\n")
	b.WriteString("# TYPE modelmux_requests_by_provider counter\n")
	...
	for _, pid := range providers {
		b.WriteString(fmt.Sprintf("modelmux_requests_by_provider{provider=%q} %d\n", pid, metrics.requestByProvider[pid].Load()))
	}

	// Average latency per provider
	b.WriteString("# HELP modelmux_avg_latency_ms Average latency per provider in milliseconds.\n")
	b.WriteString("# TYPE modelmux_avg_latency_ms gauge\n")
	for _, pid := range providers {
		sum := metrics.latencySum[pid].Load()
		count := metrics.latencyCount[pid].Load()
		if count > 0 {
			avg := float64(sum) / float64(count)
			b.WriteString(fmt.Sprintf("modelmux_avg_latency_ms{provider=%q} %.2f\n", pid, avg))
		}
	}
	metrics.mu.RUnlock()
	...
	b.WriteString("# HELP modelmux_active_providers Number of enabled providers.\n")
	b.WriteString("# TYPE modelmux_active_providers gauge\n")
	b.WriteString(fmt.Sprintf("modelmux_active_providers %d\n", enabledCount))

	// Federation info
	if fed != nil && fed.IsEnabled() {
		pool := fed.GetTrustPool()
		b.WriteString("# HELP modelmux_federation_nodes Number of federation nodes.\n")
		b.WriteString("# TYPE modelmux_federation_nodes gauge\n")
		b.WriteString(fmt.Sprintf("modelmux_federation_nodes %d\n", len(pool.Nodes)))
	}
	...
	if eventBus != nil {
		...
		if clients, ok := stats["connected_clients"].(int); ok {
			b.WriteString("# HELP modelmux_sse_clients Number of connected SSE clients.\n")
			b.WriteString("# TYPE modelmux_sse_clients gauge\n")
			b.WriteString(fmt.Sprintf("modelmux_sse_clients %d\n", clients))
		}
	}
