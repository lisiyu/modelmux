...
// to the federation.  A provider is shared when:
//   - the per-provider config key "federation_share.<provider_id>" is "true", OR
//   - relay mode is enabled (in which case every enabled provider is shared).
//
...
func (f *FederationManager) getLocalSharedProviders() ([]string, []SharedProvider) {
	...
	slog.Debug("collected local shared providers", "count", len(providers))
