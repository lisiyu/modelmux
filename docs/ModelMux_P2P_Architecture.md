...
> **版本**: v4.0.1 · **日期**: 2026-07-11 · **状态**: 架构设计（v4.0.1 重大修订）  
> **当前基线**: OpenModelPool Agent v3.3.0 (Go, 单二进制, 双模式架构)
...
| **v4.0 (本次)** | **个人版优先 + 共享网络按需加入** | 双模式、助记词身份、贡献积分、传输路径加密 | **默认离线个人代理 → 主动升级为共享节点** |

这一升级的本质变化：**默认启动永远是个人版（纯本地代理）**；只有配置 Provider Token、开启额度管理且本月有剩余额度时，才温和提示加入共享网络。加入共享网络后才生成助记词和 Node ID。**三条原则**：配置 Token ≠ 加入共享网络、有剩余额度 ≠ 自动共享、加入共享网络 ≠ 共享全部额度。
...
| 激励机制 | Tit-for-Tat | 无偿志愿 | Pinning 服务费 | **贡献积分（Contribution Credit）+ TFT 变体** |
...
## 6. 贡献激励（v4.0：Contribution Credit，不可提现/交易）
...
## Public Global Key 设计澄清（v4.0：四重限速，不保证可用）
...
## 17. P2P 网络基础模型（v4.0：仅 Network Mode）
...
## 19. BitTorrent 网络精髓借鉴（v4.0：仅共享版模式适用）
