...
| **全球统一 base URL** | P1 | network.modelmux.network + 智能路由引擎 + DNS 解析 |
...
| v4.0 (Phase 1) | 固定 2 跳 | QUIC/WS | TLS + 单层 AES-GCM | Bootstrap | 基础积分 | v3.3 |
| v4.1 (Phase 2) | 洋葱 2–3 跳 | QUIC/WS | TLS + 多层 AES-GCM + E2E | DHT + Bootstrap | 签名密钥 + 试用池 + 动态阈值 + 开放 key | v4.0 |
...
graph TB
    subgraph "终局愿景：AI 能力互联网"
        GKEY[全球统一公共 Key<br/>mk_open_global_xxxxx] -->|统一入口| GURL[全球统一 Base URL<br/>https://network.modelmux.network/v1]
...
| 目标 | 说明 |
|------|------|
| 全球统一 Base URL | `https://network.modelmux.network/v1`——一个地址，所有资源 |
...
client = openai.OpenAI(
    base_url="https://network.modelmux.network/v1",
...
| 全球统一 base URL（智能路由） | Phase 4 | network.modelmux.network + 智能路由引擎 |
