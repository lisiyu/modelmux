...
|------|:---------:|:---------:|:---------:|:---------:|
| 访问本机 Provider | ✅（ShareToPool=true） | ✅（AllowGuest=true） | ✅（AllowGuest=true） | ✅ |
| 访问其他节点 Provider | ✅（ShareToPool=true） | ✅（ShareToPool=true） | ✅（ShareToPool=true） | ✅ |
| 查看管理面板 | ❌ | ❌ | ✅ | ✅ |
| 编辑 Provider | ❌ | ❌ | ✅ | ✅ |
| 发放 Guest Key | ❌ | ❌ | ❌ | ✅ |
| 管理共享网络 | ❌ | ❌ | ❌ | ✅ |

> 括号内为该 Key 类型需要满足的 Provider 标记条件。Admin Key 不受任何标记限制。
...
每个 Provider 有两个独立的访问控制开关：

| 标记 | 默认值 | 控制范围 |
|------|-------|---------|
| **AllowGuest** | `true` | 是否允许 Guest Key 访问此 Provider |
| **ShareToPool** | `true` | 是否将此 Provider 共享到网络池 |

两个开关的组合效果：

| AllowGuest | ShareToPool | 谁能访问 |
|:---------:|:---------:|----------|
| ✅ | ✅ | Guest Key + Public Key（共享模式下）|
| ✅ | ❌ | 仅本机 Guest Key |
| ❌ | ✅ | 仅 Public Key（共享模式下）|
| ❌ | ❌ | 仅 Admin Key |
