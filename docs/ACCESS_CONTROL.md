...
> 共享模式下，标记为 shared 的上游 Key 自动贡献到网络池，Guest Key（无论 Consumer 还是 Collaborator）自动获得跨节点访问权。
...
当 Guest Key 或 Admin Key（Proxy Key）请求的模型在当前节点不存在时，系统按节点列表顺序遍历，找到第一个拥有该模型的可用节点即发起请求，消耗目标节点的共享额度池。
...
| `hasNonPrivateKey` | provider.go | 检查 Provider 是否至少有一个非私有的上游 Key |
