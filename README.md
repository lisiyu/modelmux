...
**个人模型代理 + 极客共享网络** — 先做你的本地 AI 管家，再连入全球算力共享网络。
...
**OpenModelPool 是一个临时 Token 银行 + 极客共享网络。**

默认情况下，它是一个**纯本地的个人模型代理**——管理你的 API Token，提供统一的 OpenAI 兼容接口，统计调用额度。不联网，不共享，不生成任何网络身份。

只有当你配置了 Provider Token、开启了额度管理，并且系统检测到你本月有闲置额度时，才会温和地提示你：**是否要将部分闲置额度共享到网络？**

> 你的 GPT-4o 额度本月只用了 60%？剩余 40% 过期浪费。
> 共享到 OpenModelPool 网络，别人实际使用后你获得**贡献积分**。
> 未来你需要 Claude-3 时，用贡献积分取回等价资源。
> 如果你永远不取回——这些贡献自然成为对社区的极客共享。
...
> **v4.0 修正**：当年 BT 网络里，做种者 anonymous 也能参与。但在 OpenModelPool 中，加入共享网络需要身份（助记词 → Ed25519 密钥对 → Node ID），贡献积分与身份绑定。这不是为了审查，而是为了防女巫攻击——确保贡献可追溯，防止一个人伪装成一千个节点刷积分。
>
> 另外，我们提供了一个 **Public Global Key**（公共体验密钥），任何人都可以用它体验网络中的模型能力。这不是 BT 类比的一部分——它是一个低额度的体验入口，四重限额保护，不保证 always available，但让你零门槛感受网络的价值。

---

## 🔄 模型能力交换网络（可选加入）

> **⚠️ 以下内容描述的是共享网络模式（Network Mode），默认不启用。**
> 默认的个人版（Personal Mode）只做本地代理，不参与任何网络活动。
...
当你主动加入共享网络后，经济模型是——**不是买卖，是交换**：

- 你贡献自己富余的模型能力（Gemini、Claude、GPT-4……）
- 获得**贡献积分（Contribution Credit）**，可以用来调用别人分享的模型（GLM、通义、Kimi……）
- 你贡献得越多，路由优先级越高，享受的服务质量越好
- 贡献积分不可提现、不可交易，仅用于网络内互换

```
  你的节点                    共享网络                    别人的节点
┌──────────┐            ┌──────────────┐            ┌──────────┐
│ Gemini   │────分享────→│ 贡献积分      │←──分享────│ GLM      │
│ 余量丰富  │            │ (Contribution │            │ 余量丰富  │
│ 缺 GLM   │←──调用────│  Credit)      │────调用───→│ 缺 Gemini│
└──────────┘            └──────────────┘            └──────────┘
```
...
OpenModelPool 从一个轻量级个人 AI 代理起步，逐步演化为一个 **去中心化 AI 算力共享网络**：
...
- **Phase 0** ✅ 个人版 MVP（当前） — 34+ 平台统一接入，4 维路由，多用户管理，本地额度管理
- **Phase 1** 🔜 共享版最小闭环 — 双模式架构，助记词身份，单跳中继，贡献积分，两级开关
- **Phase 2** 🌐 P2P 增强 — 多跳路由，传输路径加密，P2P 能力发现，能力验证协议
- **Phase 3** 🧠 自治网络 — 声誉系统，公证人去中心化演进，完全无中心依赖，全球自治

> 详见 [ROADMAP_v3.md](ROADMAP_v3.md)
