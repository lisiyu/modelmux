...
func GetDisclaimer() DisclaimerResponse {
	return DisclaimerResponse{
		...
		Sections: []DisclaimerSection{
			{
				Heading: "什么是共享网络？",
				Content: "共享网络允许您将部分 AI 模型能力分享给全球其他用户，同时也可以使用他人分享的模型能力。每个加入共享网络的节点都可以作为 relay 为他人转发请求，形成去中心化的 P2P 网络。",
			},
			{
				Heading: "启用后将发生什么？",
			...
			},
			{
				Heading: "⚠️ 风险警告",
				IsRisk:  true,
				Content: "• 部分 AI 平台的服务条款可能禁止 API 代理或转售行为，启用共享网络可能导致您的 API 账号被封禁\n• 虽然系统会实施速率限制和行为模拟，但无法完全消除平台检测风险\n• 您分享的计算资源可能被他人生成不当内容，您需承担相应平台的风控后果\n• 不同司法管辖区的法律法规可能限制跨区域 AI 服务的使用",
			},
		},
